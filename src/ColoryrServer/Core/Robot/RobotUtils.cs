    ...
    private static void Message(byte type, object data)
    {
        switch (type)
        {
            ...
            case 116:
                ...
                DllManager.DllRun.RobotGo(new RobotRequest(RobotRequest.MessageType.stranger, pack8.qq, 0, pack8.id, pack8.name, pack8.message, robot));
                break;
            case 123:
                var pack9 = data as StrangerMessagePostSendEventPack;
                DllManager.DllRun.RobotGo(new RobotAfter(RobotAfter.MessageType.stranger, pack9.qq, pack9.id, 0, pack9.res, pack9.error, pack9.message, robot));
                break;
        }
    }
